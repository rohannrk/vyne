...
export function CheckboxPreview() {
  ...
  const dials = useDialKit("Checkbox", {
    variant: baseline?.variant ?? "default",
    size: baseline?.size ?? "default",
    shape: baseline?.shape ?? "square",
    label: baseline?.label ?? "Accept terms",
    disabled: baseline?.disabled ?? false,
    isLoading: baseline?.isLoading ?? false,
    borderRadius: [baseline?.borderRadius ?? 4, 0, 24],
    ...
    <label className="flex items-center gap-2 text-sm">
      ...
      <span
        style={{
          ...
          fontWeight: Number(dials.fontWeight) || 400,
        }}
      >
        {dials.label}
      </span>
